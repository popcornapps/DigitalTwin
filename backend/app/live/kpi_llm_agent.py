...
# client construction below, not the actual per-call client.invoke(...)
...
def _get_client():
    ...
    with _client_init_lock:
        ...
        try:
            ...
            # .invoke() on this returns a plain parsed dict (the schema isn't
            # a Pydantic class), same shape json.loads used to produce before
            # this module used LangChain. with_structured_output requires a
            # top-level 'title' on a raw JSON schema (used as the underlying
            # function name) - _RESPONSE_SCHEMA only carries that name nested
            # under json_schema.name today, so it's added here rather than
            # duplicating/restructuring the schema itself.
...
def generate_kpi_reasoning(context: dict) -> dict:
    ...
    try:
        ...
        parsed = client.invoke(
            ...
            config={
                'callbacks': [handler],
                'run_name': f'kpi-reasoning-{batch_id}',
